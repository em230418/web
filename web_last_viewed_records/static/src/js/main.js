openerp.web_last_viewed_records = function (instance) {
    ...
    instance.web.ActionManager.include({
        ...
        add_last_viewed: function (item) {
            if (
                _.any(this.last_viewed, function (x) {
                    if (
                        x.title != item.title ||
                        ...
                        return false;
                    if (x.view_type == "form" && x.url.id != item.url.id) return false;
                    return true;
                })
            )
                return;
        ...
        get_last_viewed_title: function () {
            ...
            for (var i = 0; i < this.last_viewed.length; i += 1) {
                ...
                if (!icon && /\.settings/.test(model)) icon = "c";
                if (icon)
                    icon = _.str.sprintf(
                        '<span class="oe_last_viewed_icon fa %s"></span>',
                        icon
                    );
                titles.push(
                    _.str.sprintf(
                        '<a title="%s" href="#%s" class="oe_last_viewed_item %s">%s&nbsp;%s&nbsp;<span class="oe_e %s"/></a>',
                        title,
                        url,
                        (selected && "selected") || "",
                        icon || "",
    ...
    instance.web.ViewManagerAction.include({
        try_add_last_viewed: function (view_type) {
            ...
            if (view_type == "form" && !url.id) return false;
        ...
        do_create_view: function (view_type) {
            ...
            var exec = function () {
                if (
                    ...
                    self.try_add_last_viewed(view_type)
                ) {
        ...
        update_last_viewed_title: function () {
            this.$el
                .find(".oe_view_manager_last_viewed")
                .html(this.get_action_manager().get_last_viewed_title());
        },
        set_title: function () {
            this._super.apply(this, arguments);
            if (this.action.target != "new") this.update_last_viewed_title();
        },
    });
    instance.mail.Wall.include({
        ...
        update_last_viewed_title: function () {
            this.$el
                .find(".oe_view_manager_last_viewed")
                .html(this.ActionManager.get_last_viewed_title());
        },
    });
};
